 ...
 *
 * 核心语义 (M2 实测结论落地):
 *   1. 双后端: source 以 rtsp:// 开头走 FFmpeg/TCP (狗), 其余按 V4L2 设备 (x86 Logi)
...
public:
     ...
     * @param source       视频源: "rtsp://..." 或 V4L2 设备 ("0" / "/dev/video0")
     * @param reconnectMs  断流后重连间隔 (毫秒), <=0 时取 3000
     * @param requestWidth V4L2 请求宽, 0=相机默认 (RTSP 源忽略)
     * @param requestHeight V4L2 请求高, 0=相机默认 (RTSP 源忽略)
     */
