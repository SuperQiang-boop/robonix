...
Video sources accept RTSP, HTTP, and HTTPS stream URLs through OpenCV's FFmpeg backend, or V4L2 device numbers/paths such as `0` and `/dev/video0`. For example, set `video.source` in `vsis_video_service/config/cfg.yaml` to `http://camera-host:8080/video.mjpg`. The HTTP(S) endpoint must provide a continuous video stream in a format supported by the installed OpenCV/FFmpeg build (for example, MJPEG); a normal web page or a single still-image URL is not necessarily a video stream. HTTPS additionally depends on FFmpeg TLS and certificate support on the target system. Network capture open/read operations use five-second timeouts before the existing reconnect loop retries the source.
