...
bool VideoCameraReader::openBackend(cv::VideoCapture& cap) {
    const bool isRtsp = m_source.rfind("rtsp://", 0) == 0;
    const bool isHttp = m_source.rfind("http://", 0) == 0 ||
                        m_source.rfind("https://", 0) == 0;
    if (isRtsp || isHttp) {
        if (isRtsp) {
            // M2 验证结论: TCP 必须在 open 之前经环境变量声明, 晚了静默走 UDP, 网络一抖就花屏
            setenv("OPENCV_FFMPEG_CAPTURE_OPTIONS", "rtsp_transport;tcp", 1);
        }
        // 网络源读写超时避免断流时采集线程永久阻塞, 影响重连和 stop()
        const std::vector<int> params = {
            cv::CAP_PROP_OPEN_TIMEOUT_MSEC, 5000,
            cv::CAP_PROP_READ_TIMEOUT_MSEC, 5000,
        };
        return cap.open(m_source, cv::CAP_FFMPEG, params);
    }
