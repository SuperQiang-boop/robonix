...
User-invocable Robonix skill for allowlisted Unitree G1 preset upper-arm
actions.

## Capability

- `robonix/skill/g1_arm_actions/run`: MCP tool that accepts an `action` string.
...
The skill resolves `robonix/primitive/arm/action` from the `g1_arm` primitive
and calls it over gRPC.
